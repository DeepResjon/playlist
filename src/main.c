...
#define PLAYLIST_IN_PATH "include/playlist.txt"
...
Node **load_file(const char *filename, Node **list) {
  ...
  while (
    ...
    // YOUR CODE HERE
    strncpy((char *)new_node->data, line, TRACK_TITLE_SIZE);
...
void print_tracks(const Node *const playlist) {
  Node const *current = playlist;
  for (long i = 1; current; i++, current = current->next)
    printf("%2ld: %s\n", i, (char *)current->data);
}
